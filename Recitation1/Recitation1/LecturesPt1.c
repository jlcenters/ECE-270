...
int main() {
	...
	fprintf(fp, "%s", filetext);
	fclose(fp);
	
	char textfromfile[1024];
	fp = fopen(filename, "r");
	if (fp == NULL) {
		perror("There was an error opening the file for reading");
		return 0;
	}
	
	if (fgets(textfromfile, sizeof(textfromfile), fp) != NULL) { 
		printf("File contents: %s", textfromfile);

		if (strcmp(textfromfile, filetext) == 0) { 
			printf("Verification: Success [TEXT MATCH]\n");
		}
		else {
			printf("Verification: Failure [TEXT DOES NOT MATCH]\n");
		}
	}
	else {
...
int main() {
	...
	//define as pointers to strings; allows program to run faster
	const char* filename = "filename.txt"; 
	const char* text = "Hello!";

	//creates a file pointer as a reference to filename (opens as a WRITE-ONLY, will overwrite anything already in file)
	FILE* fp = fopen(filename, "w");  //if file doesn't exist, file will be created using the name provided
	//error check
	if (fp == NULL) {
		perror("Error opening file for writing"); //prints error message
		return 1; //exits main; ends program
	}
	fprintf(fp, "%s", text); //prints text into designated file
	fclose(fp); //close after each action; otherwise in the case of a program crash the file remains open until the OS restarts

	charbuffer[1024]; //creates a buffer with the size up to 1024 bits

	fp = fopen(filename, "r"); //stores ref pointing to READ-ONLY
	//error check
	if (fp == NULL) {
	...
	if (fgets(buffer, sizeof(buffer), fp) != NULL) { //will read and store into buffer; will read only the length defined (in this case, length is the size of buffer)
		printf("File contents: %s", buffer); //prints file contents to console

		//the following if/else statement is to validate a successful read of the file
		if (strcmp(buffer, text) == 0) { //checks to ensure the data collected by fgets() is the data from the original definition
			printf("Verification: Success [TEXT MATCH]\n");
		}
		else { //for some reason the text does not match the expected value
			printf("Verification: Failure [TEXT DOES NOT MATCH]\n");
		}
	}
	else { //will usually occur in the case of a null file
		printf("File is empty or an error occurred while reading.\n");
	}
	fclose(fp);
